...
| Config takeover service | Claude/Codex/OpenCode native Windows paths, custom paths, WSL UNC paths, sidecar `.aiusage.bak` backups, idempotent activation, restore, OpenCode JSONC parsing, and Tauri commands are implemented; WSL writes skip Windows ACL tightening and remain explicit `wslDistribution` targets |
| Config takeover UI | Windows UI shows Claude/Codex/OpenCode native and WSL target paths, managed state, backup state, parse warnings, JSONC status, apply actions driven by proxy drafts, and gated restore actions for existing managed or backed-up files |
