...
| Config takeover service | Claude/Codex/OpenCode native Windows paths, custom paths, sidecar `.aiusage.bak` backups, idempotent activation, restore, OpenCode JSONC parsing, and Tauri commands are implemented; WSL distro target status is reported through explicit `wslDistribution` targets but not written without a future WSL activation path |
| Config takeover UI | Windows UI shows Claude/Codex/OpenCode native and WSL target paths, managed state, backup state, parse warnings, JSONC status, native/custom apply actions driven by proxy drafts, and gated restore actions for existing native/custom managed or backed-up files |
| Proxy runtime | Async Rust passthrough supervisor starts/stops local listeners, exposes health through Tauri, validates client keys, normalizes `/v1` upstream paths, injects upstream auth, streams upstream responses, emits request/usage events, parses OpenAI/Anthropic usage shapes, and surfaces four-track health plus preflight/start/stop controls in the Windows UI |
...
| Credential registry | Structured provider credentials are stored in Windows Credential Manager/DPAPI through the platform vault, exposed as secret-free summaries through Tauri, and managed in the Windows UI through save/delete controls that never reveal stored secrets |
