...
The current Windows line includes the app shell, schema/fixture harness, core Windows adapters, sensitive-file permission tightening, Claude/Codex/OpenCode config takeover services with backup/restore semantics, and an async passthrough proxy supervisor exposed through Tauri commands and UI health. The proxy runtime emits request/usage events, parses OpenAI/Anthropic token usage shapes, and persists per-track usage archive files. Later phases fill in protocol conversion depth, provider refresh, full UI parity, installer/signing, and release automation.
